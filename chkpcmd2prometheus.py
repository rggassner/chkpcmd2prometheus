...
def get_asg_perf(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command("timeout 5 asg perf -v -p", bufsize=-1, timeout=CONNECTION_TIMEOUT, get_pty=True)
        ...
        for line in lines:
            ...
            if "Performance Summary" in line:
                ...
                if len(parts) >= 2 and 'Name' not in line and 'Value' not in line and 'IPv4' not in line:
                    key, value = parts[1:3]
                    key = key.strip()
...
def get_asg_conns(host, user, password):
...
def format_metrics():
    ...
    for host, metrics_data in metrics_cache.items():
        ...
        for member, values in data["members"].items():
            results.append(f'c2p_asg_conns_fw1_vals{{host="{host}", host_name="{metrics_data["name"]}", member="{member}"}} {values.get("vals", 0)}')
            results.append(f'c2p_asg_conns_fw1_peak{{host="{host}", host_name="{metrics_data["name"]}", member="{member}"}} {values.get("peak", 0)}')
            results.append(f'c2p_asg_conns_fw1_slinks{{host="{host}", host_name="{metrics_data["name"]}", member="{member}"}} {values.get("slinks", 0)}')
            results.append(f'c2p_asg_conns_sxl_connections{{host="{host}", host_name="{metrics_data["name"]}", member="{member}"}} {values.get("number_of_connections", 0)}')

        # Total metrics
        results.append(f'c2p_asg_conns_fw1_total_connections{{host="{host}",host_name="{metrics_data["name"]}"}} {data.get("fw1_connections_total", 0)}')
        results.append(f'c2p_asg_conns_sxl_total_connections{{host="{host}",host_name="{metrics_data["name"]}"}} {data.get("securexl_connections_total", 0)}')
                ...
                for key in keys_to_check:
                    if key in perf_data['Per Path Distribution']:
                        for path_type, value in perf_data['Per Path Distribution'][key].items():
                            ...
                            # The value is just a float, not a dictionary with items
                            results.append(f'c2p_asg_perf_path_{index_name}{{host="{host}", host_name="{metrics_data["name"]}", path="{path_type}"}} {value}')

            # SGM distribution metrics
            if "Per SGM Distribution" in perf_data and len(perf_data['Per SGM Distribution']) > 0:
                keys_to_check = [
                ...
                for member, values in perf_data['Per SGM Distribution'].items():
                    for key in keys_to_check:
                        if key in values:
                            index_name = key.replace(' ', '_').replace('.', '').lower()
                            results.append(f'c2p_asg_perf_sgm_{index_name}{{host="{host}", host_name="{metrics_data["name"]}", member="{member}"}} {values[key]}')

            # Performance summary metrics
            if "Performance Summary" in perf_data and len(perf_data['Performance Summary']) > 0:
                keys_to_check = [
                    'Acceleration load avg', 'Acceleration load max', 'Acceleration load min',
                    'Concurrent connections', 'Connection rate', 'Instances load avg',
                ...
                for key in keys_to_check:
                    if key in perf_data['Performance Summary']:
                        index_name = key.replace(' ', '_').replace('.', '').lower()
                        results.append(f'c2p_asg_perf_summary_{index_name}{{host="{host}", host_name="{metrics_data["name"]}"}} {perf_data["Performance Summary"][key]}')
...
def update_metrics_loop(servers):
    ...
    while True:
        new_metrics = {}
        for host, user, password, hostname in servers:
            metrics_data = {}
